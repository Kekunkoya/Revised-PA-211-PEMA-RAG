...
# Optional prebuilt vectors at repo root:
PREBUILT = {
    "OpenAI": "openai_vectors.pkl",
    "Gemini": "gemini_vectors.pkl",
}

# Default chunking (tuned a bit tighter to boost matching)
CHUNK_SIZE = 900
OVERLAP = 250
...
if not OPENAI_API_KEY and not GEMINI_API_KEY:
    st.error("Missing both OPENAI_API_KEY and GEMINI_API_KEY in secrets. Add at least one.")
    st.stop()
...
# =========================
# Build / Load vectors (from repo root or cache)
# =========================
def build_corpus(all_text: Dict[str, str], contextual=False) -> Tuple[List[str], List[dict]]:
    ...
    for fname, full in all_text.items():
        ...
        for i, ch in enumerate(chunks):
            # include filename as a tiny hint for retrieval
...
def build_store(provider: str, all_text: Dict[str, str], contextual: bool) -> VectorStore:
    ...
    # 3) Save prebuilt to repo root (so you can commit it and never rebuild live)
    save_prebuilt(provider, store)
    return store

# Manual rebuild action (button)
        ...
        for _ in range(4):
            try:
                r = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[{"role": "system", "content": "Answer only with the provided context."},
...
def transform_query(engine: str, query: str, ttype: str) -> List[str]:
    if ttype == "rewrite":
        ...
        return [llm(engine, p)]
    if ttype == "step_back":
        p = f"Give one step-back (higher-level) version of the question.\nOriginal: {query}\nStep-back:"
        return [llm(engine, p)]
    if ttype == "decompose":
...
# UI
# =========================
st.set_page_config(page_title="PA 211 / PEMA RAG (prebuilt vectors)", layout="wide")
st.title("PA 211 / PEMA RAG — OpenAI vs Gemini vs Both (Prebuilt Vectors)")
...
def run_engine(provider: str):
    ...
    store_ctx = get_store(provider, contextual=contextual)
    ...
    if mode == "Combined":
        ...
        # contextual lookup
        hits += [h for h in store_ctx.search(query, k=top_k) if (h["source"],h["chunk_id"]) not in {(x["source"],x["chunk_id"]) for x in hits}]
        # transforms
        ...
        for tq in tqs:
            for h in store_std.search(tq, k=max(2, top_k//2)):
                key = (h["source"], h["chunk_id"])
                if key not in {(x["source"],x["chunk_id"]) for x in hits}:
                    hits.append(h)
        # sort & trim
    ...
    if mode == "Adaptive":
        qtype, hits = adaptive_retrieve(provider, store_ctx if contextual else store_ctx, query, k=top_k)
        ans = answer(provider, query, hits)
...
if st.button("Run"):
    ...
    if compare_choice == "OpenAI":
        ...
        # Both: merge top contexts from both into a single answer (OpenAI used for final synthesis)
        oai = run_engine("OpenAI")
        gem = run_engine("Gemini")
        ans_o, hits_o, tag_o = oai
        ans_g, hits_g, tag_g = gem

        # Merge by (source, chunk_id) with max similarity
        merged = {}
    ...
    if show_debug and results:
        st.markdown("---")
        st.markdown("**Debug**")
        for name, _, hits, _ in results:
            st.write(name, "top sources:", [os.path.basename(h["source"]) for h in hits])
