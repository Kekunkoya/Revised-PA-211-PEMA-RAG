# Let's generate the Streamlit app code that compares OpenAI vs Gemini RAG across multiple modes
...
def run_pipeline(provider: str, pipeline: str):
    if provider == "openai" and not st.session_state.store_openai:
        return None, [], {}
    if provider == "gemini" and not st.session_state.store_gemini:
        return None, [], {}

    store = st.session_state.store_openai if provider=="openai" else st.session_state.store_gemini
...
if go:
    ...
    with col1:
        ...
        if st.session_state.store_openai:
            ans_oai, hits_oai, meta_oai = run_pipeline("openai", pipeline)
            time.sleep(pause_s)
    ...
    with col2:
        ...
        if st.session_state.store_gemini:
            ans_gem, hits_gem, meta_gem = run_pipeline("gemini", pipeline)
            time.sleep(pause_s)
...
st.markdown("---")
st.caption("Built from your notebooks: context-enriched (G04/04), query-transform (G07/07), adaptive (G12/12) — unified into one app.")
'''

# Write to /mnt/data/app.py
...
out_path, "/mnt/data/requirements.txt"
